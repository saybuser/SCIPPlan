...
Figure 1: Visualization of a plan generated by SCIPPlan [1,2] for the example navigation domain where the red square represents the agent, the blue square represents the block, the gold star represents the goal location and the delta represents time. The agent can control its acceleration (as well as its duration) which modifies its speed and location in a two-dimensional maze. The goal of the domain is to find a path for the agent to the goal location with minimal makespan such that the agent does not collide with the obstacle. Note that the acceleration, speed, location and the duration of the control input are all real-valued.
